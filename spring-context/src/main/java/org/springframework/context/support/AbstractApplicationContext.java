...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		try {
			...
			try {
				...
				//为防止bean资源占用在异常处理中销毁已经在前面过程中生成的单例bean
				destroyBeans();
	...
	protected void prepareRefresh() {
		...
		//判断刷新前的应用程序监听器集合是否为空,如果为空则将监听器添加到次集合中
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		//初始化一个默认bean工厂:DefaultListAbleBeanFactory
		refreshBeanFactory();
		//返回刚刚初始化好的bean工厂
		return getBeanFactory();
	}
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//设置beanFactory的语言表达处理器
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		//为beanFactory增加一个默认的PorpertyEdit,这个主要是对bean的属性等设置的一个管理的工具类
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		//添加beanPostProcessor,applicationContextAwareProcessor此类用来完成默写aware对象的注入
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		//设置忽略自动装配的接口,这些接口的实现是由容器通过set方法
		...
		//增加对AspectJ的支持,在java中织入分为三种方式,分别为:编译器织入,类加载器织入,运行期织入,编辑器织入是指在java编译器,采
		//而类加载器织入则指通过特殊的类加载器,在类字节码加载到jvm时,织入切面,运行期织入则是采用jdk和cglib进行切面的织入
		//AspectJ提供了2种织入方式,第一种通过特殊编译器,在编译器,将aspectj语言编写的切面织入到java类中,第二种是类加载器织入
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		//如果beanFactory之前没有嵌入值解析器,则注册默认的嵌入值解析器,主要用于注解属性值的解析
